...
public class Series2 {
  ...
  public static String exportWordsWithoutALetter(List<String> names, char letter) {
    ...
    for (String name : names) {
      if (name != null
          && !name.toLowerCase(Locale.ENGLISH).contains(String.valueOf(letter).toLowerCase(Locale.ENGLISH))) {
        list.add(name);
      }
    }
  ...
  public static List<String> findAnagrams(String name) {
    ...
    anagrams = scramble(name, "", anagrams);
    return  anagrams;
  }

  private static List<String> scramble(String input, String sofar, List<String> anagrams) {
    if (input.equals("")) {
      anagrams.add(sofar);
    }
    for (int i = 0; i < input.length(); i++) {
      ...
      if (input.indexOf(c, i + 1) != -1)
        continue;
      scramble(input.substring(0, i) + input.substring(i + 1), sofar + c, anagrams);
    }
    return anagrams;
  }
  // Fin des embrouilles
